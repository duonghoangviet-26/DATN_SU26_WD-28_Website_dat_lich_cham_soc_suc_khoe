  ...
  await SinhHieuKham.findOneAndUpdate(
    { appointment_id: appointmentId },
    {
      $set: {
        ...
        can_nang, chieu_cao, huyet_ap, nhiet_do, nhip_tim,
        nguoi_do_id: nurseId,
...
export async function createDraft(req, res) {
  try {
    const { appointment_id, chan_doan, huong_dan_dieu_tri, ghi_chu, trieu_chung_ban_dau, ghi_chu_dieu_duong, ngay_tai_kham, sinh_hieu } = req.body
    ...
    if (['cancelled', 'no_show'].includes(a.status)) {
      return fail(res, 409, 'Không thể nhập hồ sơ cho lịch hẹn đã hủy hoặc không đến')
    }

    const exists = await KetQuaKham.exists({ appointment_id: a._id })
    if (exists) return fail(res, 409, 'Hồ sơ khám đã tồn tại cho lịch hẹn này')
...
export async function update(req, res) {
  try {
    ...
    if (ngay_tai_kham !== undefined) {
      const a = await LichHen.findById(result.appointment_id).select('ngay_kham member_id').lean()
      if (ngay_tai_kham && !isNgayTaiKhamHopLe(ngay_tai_kham, a.ngay_kham)) {
        return fail(res, 400, 'Ngày tái khám phải từ ngày tiếp theo sau ngày khám')
      }
    ...
    if (sinh_hieu) {
      const a = await LichHen.findById(result.appointment_id).select('member_id').lean()
      await upsertVitals(result.appointment_id, a?.member_id, req.user.id, sinh_hieu)
    }
