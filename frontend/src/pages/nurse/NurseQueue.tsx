...
import type { NurseQueueItem, AppointmentStatus } from '@/types'
...
const STATUS_COLOR: Record<string, 'yellow' | 'blue' | 'green' | 'red' | 'gray'> = {
  ...
  waiting_doctor_confirm: 'yellow', completed: 'green',
}
...
export default function NurseQueue() {
  ...
  useEffect(() => {
    setLoading(true)
    setError(false)
    nurseService.getQueue({ date: filterDate, status: filterStatus })
      .then(setItems)
      .catch(() => setError(true))
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Bệnh nhân đã check-in hoặc đang trong luồng khám thuộc ca của bạn — sắp theo giờ hẹn."
      />
      ...
      <div className="mb-3 flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          >
            ...
            <option value="waiting_doctor_confirm">Chờ bác sĩ xác nhận hồ sơ</option>
                ...
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-base font-medium text-slate-500">Chưa có bệnh nhân check-in nào khớp bộ lọc.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  items.map((q) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-600">{q.ten_dich_vu ?? '—'}</td>
                      <td className="px-4 py-3 text-slate-600">{q.loai_kham === 'clinic' ? 'Tại phòng khám' : 'Tại nhà'}</td>
                      <td className="px-4 py-3">
