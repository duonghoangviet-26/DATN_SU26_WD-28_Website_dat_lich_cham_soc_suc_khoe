...
import { nurseMenu } from '@/routes/nurseMenu'
...
export default function NurseSidebar({ open, onClose }: Props) {
  return (
    <>
      {open && (
      ...
      >
        <div className="flex h-16 shrink-0 items-center gap-3 border-b border-slate-100 px-5">
          ...
          <div>
            ...
            <p className="text-[10px] font-semibold uppercase tracking-widest text-brand-500">Nurse Panel</p>
          </div>
        </div>

        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {nurseMenu.map((item, idx) => {
        ...
        <div className="shrink-0 border-t border-slate-100 px-4 py-3">
          <p className="text-[10px] text-slate-400">VitaFamily v1.0 · Cổng y tá</p>
        </div>
      </aside>
    </>
  )
