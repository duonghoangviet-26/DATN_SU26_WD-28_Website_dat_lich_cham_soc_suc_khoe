...
- ✅ **Trang Điều phối bác sĩ ĐÃ TRIỂN KHAI 2026-08-23** (chi tiết: `docs/Dieu phoi bac si - trien khai (2026-08-23).md`): tách `services/rescheduleRules.js` (hàm thuần, có unit test) — `locked` nay phân biệt rõ ba nghĩa, `apDungPhuongAn` **không bao giờ** chiếm slot có `bi_khoa_boi_nghi_phep`; khách **tự dời** thì slot cũ **trả về pool** (`khoaSlotCu: false`), chỉ `phong_kham` mới giữ khoá. Khoá slot vì nghỉ nay bắt cả slot đang **giữ sẵn cho đề xuất khác** rồi **sinh lại phương án** cho lịch đó (`sinhLaiDeXuatChoLichMatCho`). Bộ lọc `!de_xuat_doi` đổi thành `nenSinhLaiDeXuat()` — đề xuất còn mở được sinh lại thay vì bị bỏ qua. `sinhPhuongAnDoi` mở rộng **7 ngày** (`DOI_LICH_SO_NGAY_TIM`), phạt **480′/ngày** (`DOI_LICH_PHAT_NGAY_PHUT`) nên cùng ngày luôn xếp trước ngày mai. Thêm **khôi phục báo nghỉ** (`PATCH /receptionist/doctor-leaves/:id/huy-bao-nghi`, dùng `trang_thai='da_huy'` sẵn có, KHÔNG đổi schema) — lịch **đã dời xong giữ nguyên** ở chỗ mới. Trang `/receptionist/dieu-phoi/:leaveId` dạng bảng + duyệt hàng loạt (`POST .../bulk-approve`, mỗi lịch một giao dịch riêng, trần 100/lượt). **ĐÃ GỠ** `bulkRescheduleAppointments` (crash 100% + bỏ qua rule mục 5/11/15).
...
- ✅ **Gộp "Quản lý và điều phối" (2026-08-25)**: 2 mục menu lễ tân cũ ("Điều phối bác sĩ" + "Điều phối lịch hẹn") gộp thành 1 trang 2 tab tại `/receptionist/quan-ly-dieu-phoi`. Kèm 5 sửa lỗi backend: hợp nhất `reportDoctorUnavailable`/`duyetDonNghi` (trước đó là 2 bản cài đặt song song, vi phạm nguyên tắc mục 7), chặn lễ tân báo nghỉ vượt thẩm quyền >1 ngày, lọc đơn `da_huy` khỏi danh sách điều phối, thêm API xem trước ảnh hưởng trước khi báo nghỉ, và tạo việc "cần liên hệ tay" cho nhóm 0-phương-án (trước đó bị bỏ sót hoàn toàn khỏi `/contact-tasks`). Chi tiết: `docs/Quan ly va dieu phoi - trien khai (2026-08-25).md`, spec: `docs/superpowers/specs/2026-08-25-quan-ly-va-dieu-phoi-design.md` (local-only).
...
- ✅ **Đã chốt (2026-08-23, xác nhận lại 2026-08-25 khi rà đợt "Quản lý và điều phối"):** slot cũ khi khách TỰ dời trước `T-30'` → theo mục 11, TRẢ VỀ POOL để bán lại (`capNhatSlotCuSauKhiDoi(false)` trong `rescheduleRules.js`). Chỉ khi `ly_do_doi='phong_kham'` (phòng khám chủ động dời, mục 15) mới `locked` vĩnh viễn. Hai mục nói về hai tình huống khác nhau và không còn mâu thuẫn trong code.
