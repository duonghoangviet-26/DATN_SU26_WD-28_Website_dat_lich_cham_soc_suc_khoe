...
### A5 — Đặt lịch khám
- 3 hình thức: `clinic` (tại viện) · `home` (bác sĩ đến nhà) · `video` (tư vấn video)
- Luồng: Chọn chuyên khoa/bệnh viện → Chọn bác sĩ → Xem slot còn trống → Chọn giờ → Thanh toán
- Có thể đặt cho bản thân hoặc thành viên gia đình
- Appointment `unpaid` quá 15 phút → cron job tự hủy
- `gia_kham` = snapshot `phi_tu_van` lúc đặt (không đổi khi bác sĩ thay đổi giá sau)
- **Bảng liên quan:** `appointments`, `slots`, `payments`
...
- Tối đa nộp lại 5 lần
- Khi duyệt: `users.role` đổi thành `'doctor'` (cùng transaction)
- Bác sĩ bị từ chối nhận email kèm lý do
- **Bảng liên quan:** `doctors`, `doctor_specialties`, `doctor_hospitals`

### B2 — Quản lý lịch làm việc
- Bác sĩ tạo khung giờ khám (`slots`) cho từng ngày
- Slot đầy (= `so_benh_nhan_hien_tai` đạt `so_benh_nhan_toi_da`) → tự khóa
- Cập nhật `so_benh_nhan_hien_tai` dùng `SELECT FOR UPDATE` tránh race condition
- **Bảng liên quan:** `doctor_schedules`, `slots`
...
- CRUD bệnh viện và chuyên khoa
- Ẩn thay vì xóa (`status = 'hidden'`) để không ảnh hưởng dữ liệu cũ
- **Bảng liên quan:** `hospitals`, `specialties`
...
- **Payout:** cuối tháng, Admin chuyển 85% cho bệnh viện (giữ 15% hoa hồng)
- Chính sách hoàn tiền lấy từ bảng `payment_settings` (không hardcode)
- **Bảng liên quan:** `payments`, `refunds`, `payouts`, `payment_settings`
